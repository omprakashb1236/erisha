...
export default function LeadsTool() {
  ...
  const q = `*[_type == 'optOutLead'] | order(_createdAt desc)`;
  ...
  const handleClick = (sortBy: string) => {
    ...
    const sortClause = sortBy === "newest" ? "_createdAt desc"
      : sortBy === "oldest" ? "_createdAt asc"
        : sortBy === "a-z" ? "name asc"
          : sortBy === "z-a" ? "name desc"
            : "_createdAt desc";

    const q = `*[_type == 'optOutLead'${whereClause}] | order(${sortClause})`;
    console.log(q);
  ...
  const orderedLeads = leads.map((lead: Lead) => {
    return {
      ...
      email: lead.email || '',
      phone: lead.phone || '',
      stopMarketing: lead.stopMarketing || '',
    ...
    <ToastProvider>
      <Container className="mainContainer" style={{ marginTop: '60px', maxWidth: '100%', overflow: 'scroll' }}>
        <div className='headingContainer'>
          <Heading className='px-4 fs-3 text-center'>Leads Dashboard</Heading>
