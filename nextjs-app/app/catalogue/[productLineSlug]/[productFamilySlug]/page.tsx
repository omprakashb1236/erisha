...
export default async function ProductFamilyPage({
  ...
  const filterGroups = [];
  const parentLine = productFamily.productLine;
  if (parentLine) {
    if (parentLine.filterType?.length) {
      filterGroups.push({ label: "Type", paramName: "type", options: parentLine.filterType });
    }
    if (parentLine.filterWire?.length) {
      filterGroups.push({ label: "Wire / Non-Wire", paramName: "wire", options: parentLine.filterWire });
    }
